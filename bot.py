...
_REDIS_REFRESH_TS: float = 0.0


def _refresh_orders_state_from_redis(*, force: bool = False) -> None:
    """Подтянуть ORDERS и user_states из Redis, не затирая свежее in-memory состояние."""
    global _REDIS_REFRESH_TS
    if not force and (time.time() - _REDIS_REFRESH_TS) < 2.0:
        return
    _REDIS_REFRESH_TS = time.time()
    ...
    for uid, bucket in loaded_states.items():
        ...
        for key, val in bucket.items():
            if key in (
                ...
                if val is not None and local_b.get(key) is None:
                    local_b[key] = val
                continue
...
def _bind_proof_upload_session(
    uid: int, ud: dict, oid: int, *, persist: bool = True
) -> None:
    ...
    if isinstance(ud, dict):
        ...
        if pm and oid_i in ORDERS and isinstance(ORDERS.get(oid_i), dict):
            o = ORDERS[oid_i]
            if not o.get("payment_pending_method"):
                o["payment_pending_method"] = pm
    b = _user_state_bucket(uid)
    b["awaiting_proof"] = oid_i
    b["awaiting_payment_order_id"] = oid_i
    b["proof_deadline"] = int(time.time()) + 6 * 3600
    ...
    if persist:
        try:
            ...
            logging.getLogger(__name__).exception(
                "bind_proof_upload_session save_state uid=%s oid=%s", uid, oid_i
            )
...
MSG_PAY_PROOF_RECEIVED = (
    "⏳ Получили скрин. Напишите адрес доставки, ФИО и номер телефона."
)
MSG_PAY_PROOF_OK = (
    "✅ Скрин сохранён. Напишите адрес доставки, ФИО и номер телефона."
)
MSG_POSTPAID_NEED_DETAILS = (
    "Напишите адрес отделения, ФИО и номер телефона."
)
MSG_POSTPAID_SUBMITTED = (
    "✅ Ваши данные отправлены. Сейчас менеджер подтвердит заказ и свяжется с вами."
)
MSG_POSTPAID_PROCESSING = "⏳ Отправляем заказ менеджеру..."
...
MSG_POSTPAID_RESEND_PROOF = (
    "Скрин оплаты не сохранился. Пришлите фото чека ещё раз, затем снова укажите данные для доставки."
)
...
MSG_POSTPAID_SHIPPING_BY = (
    "Напишите адрес отделения Европочты, ФИО и номер телефона одним сообщением."
)
...
MSG_SITE_CHECKOUT_ALREADY_SHOWN = (
...
PAY_CARD_BODY = (
    ...
    "Имя на карте:\n"
    "DANIL PARFIONAU\n\n"
    "После оплаты пришлите 📸 скрин перевода.\n"
    "Кнопку «✅ Оплатил» нажимать необязательно."
)
PAY_TRANSFER_BODY = (
    "📱 Перевод на номер\n\n"
    "Телефон:\n"
...
def _set_awaiting_proof_session(
    uid: int, ud: dict, oid: int, *, persist: bool = True
) -> None:
    oid_i = int(oid)
    if isinstance(ud, dict):
    ...
    if persist:
        try:
            ...
            logging.getLogger(__name__).exception(
                "set_awaiting_proof_session save_state uid=%s oid=%s", uid, oid_i
            )
...
def _shipping_session_valid(uid: int, oid: int) -> bool:
    """Заказ подходит для приёма адреса (новый или доп. сообщение после отправки админу)."""
    try:
        uid_i = int(uid)
        oid_i = int(oid)
    except (TypeError, ValueError):
...
def _clear_shipping_session_keys(uid: int, user_data: Optional[dict] = None) -> None:
    if isinstance(user_data, dict):
        user_data.pop("postpaid_thread_oid", None)
        user_data.pop("awaiting_shipping_oid", None)
    for key in ("postpaid_thread_oid", "awaiting_shipping_oid", "awaiting_postpaid_shipping"):
        _user_state_pop(uid, key, persist=False)


def _resolve_postpaid_thread_oid(
    uid: int, user_data: Optional[dict]
) -> Optional[int]:
    """Активный заказ «ждём адрес» — свежий заказ со скрином важнее старой сессии."""
    ...
    if active is not None:
        for key in ("postpaid_thread_oid", "awaiting_shipping_oid"):
            ...
            if tid is not None and int(tid) != int(active):
                _user_state_pop(uid, key, persist=False)
        ud["postpaid_thread_oid"] = int(active)
        ud["awaiting_shipping_oid"] = int(active)
        return int(active)
    candidates: List[int] = []
    for src in (
    ...
    pend = _resolve_awaiting_payment_order_id(uid, ud)
    if pend is not None:
        candidates.append(int(pend))
    seen: set = set()
    preferred: List[int] = []
    fallback: List[int] = []
    for oid_i in candidates:
        if oid_i in seen:
            continue
        seen.add(oid_i)
        o = _ensure_order_in_orders(int(oid_i), int(uid)) or ORDERS.get(int(oid_i))
        if not isinstance(o, dict) or int(o.get("user_id") or 0) != int(uid):
            continue
        if o.get("paid"):
            continue
        if o.get("checkout_submitted_to_admin"):
            fallback.append(int(oid_i))
            continue
        if not _order_has_payment_proof(o):
            _recover_payment_proof_on_order(int(uid), int(oid_i))
            o = _ensure_order_in_orders(int(oid_i), int(uid)) or ORDERS.get(int(oid_i))
        if isinstance(o, dict) and _order_has_payment_proof(o):
            preferred.append(int(oid_i))
    pick = preferred[-1] if preferred else (fallback[-1] if fallback else None)
    if pick is not None:
        ud["postpaid_thread_oid"] = int(pick)
        ud["awaiting_shipping_oid"] = int(pick)
        return int(pick)
    for key in ("postpaid_thread_oid", "awaiting_shipping_oid"):
        tid = _user_state_get(uid, key)
        if tid is not None and not _shipping_session_valid(uid, int(tid)):
            _user_state_pop(uid, key, persist=False)
    return None


def _bind_postpaid_thread_oid(
    uid: int,
    user_data: Optional[dict],
    oid: int,
    *,
...
def _hydrate_postpaid_state_for_uid(uid: int) -> None:
    """Подтянуть ORDERS/user_states из Redis и восстановить чеки из USER_ORDERS."""
    try:
        uid_i = int(uid)
    except (TypeError, ValueError):
        return
    ...
    for rec in USER_ORDERS.get(uid_i) or []:
        if not isinstance(rec, dict):
            continue
        if not str(rec.get("proof_file_id") or "").strip():
            continue
        try:
            oid_i = int(str(rec.get("id") or "0"))
        except (TypeError, ValueError):
        ...
        if oid_i <= 0:
            continue
        _ensure_order_in_orders(oid_i, uid_i)
        if not _order_has_payment_proof(ORDERS.get(oid_i) or {}):
            _recover_payment_proof_on_order(uid_i, oid_i)
...
async def _reply_postpaid_finalize_to_client(
    ...
    *,
    ...
    try:
        if ok or reason == "already":
            await msg.reply_text(MSG_POSTPAID_SUBMITTED, reply_markup=REPLY_KB)
        elif reason == "no_proof":
            await msg.reply_text(MSG_POSTPAID_RESEND_PROOF)
        elif reason == "empty":
...
async def _forward_postpaid_client_payload_to_admin(
    ...
    *,
    ...
    try:
        ...
        try:
            await msg.reply_text(MSG_POSTPAID_FORWARDED_OK, reply_markup=REPLY_KB)
        except Exception:
            pass
        return True
    except Exception as e:
    ...
    *,
    ...
    try:
        ok, reason = await asyncio.wait_for(
            _finalize_order_submission_to_admin(
                context, uid, int(oid), delivery_details=details
            ),
            timeout=90.0,
        )
    except asyncio.TimeoutError:
        log.error("postpaid finalize timeout uid=%s oid=%s", uid, oid)
    ...
    if ok or reason in ("already", "ok"):
        _user_state_pop(uid, "awaiting_postpaid_shipping", persist=False)
        final_text = MSG_POSTPAID_SUBMITTED
    else:
        final_text = MSG_POSTPAID_SUBMIT_FAIL
    try:
        if processing_msg is not None:
            await processing_msg.edit_text(final_text, reply_markup=REPLY_KB)
        else:
            await msg.reply_text(final_text, reply_markup=REPLY_KB)
    except Exception:
        log.exception("postpaid collect final ack uid=%s oid=%s", uid, oid)
        try:
            await msg.reply_text(final_text, reply_markup=REPLY_KB)
        except Exception:
            pass
...
async def _finalize_order_submission_to_admin(
    ...
    *,
    ...
    uname = str(o.get("username") or "").strip().lstrip("@") or None
    try:
        await _ensure_customer_admin_folder(context, int(uid), uname)
    except Exception:
...
def _normalize_site_order_ref(raw: object) -> str:
...
def _site_checkout_push_keys(uid: int, order_ref: Optional[str]) -> List[str]:
    ...
    uid_i = int(uid)
    keys: List[str] = [f"{uid_i}:"]
    ref = _normalize_site_order_ref(order_ref)
    if ref:
        keys.append(f"{uid_i}:{ref}")
        compact = ref.replace("-", "").lower()
        if compact and compact != ref.lower():
            keys.append(f"{uid_i}:{compact}")
    pend = _normalize_site_order_ref(PENDING_SITE_ORDER_BY_USER.get(uid_i))
    if pend:
        keys.append(f"{uid_i}:{pend}")
        pend_compact = pend.replace("-", "").lower()
        if pend_compact and pend_compact != pend.lower():
            keys.append(f"{uid_i}:{pend_compact}")
    out: List[str] = []
    seen: set = set()
    for k in keys:
        if k not in seen:
...
def _resolve_site_order_ref_from_start(uid: int, start_oid: str) -> str:
    """order_13 из /start → UUID заказа с сайта, если он уже в памяти бота."""
    ref = _normalize_site_order_ref(start_oid)
    if not ref:
        return ref
    if re.fullmatch(r"\d+", ref):
        ...
        try:
            ...
            return ref
        for raw in SHARED_DEEP_LINK_ORDERS.values():
            if not isinstance(raw, dict):
                continue
            try:
                if int(raw.get("buyer_seq") or raw.get("buyerSeq") or 0) == seq:
                    oid = _normalize_site_order_ref(
                        raw.get("order_id") or raw.get("id")
                    )
                    if oid:
                        return oid
            except (TypeError, ValueError):
...
async def on_payment_method(
    ...
    try:
        ...
        _clear_shipping_session_keys(uid, ud)
        if isinstance(o, dict):
...
async def on_payment_paid(
    ...
    try:
        ...
        try:
            await q.message.reply_text(
                "Не удалось открыть шаг со скрином. Нажмите «✅ Оплатил» ещё раз.",
                reply_markup=REPLY_KB,
            )
        except Exception:
            pass
...
async def _on_payment_paid_body(
    ...
    *,
    ...
    if oid is None:
        await q.message.reply_text(
            "Не удалось открыть шаг со скрином. Нажмите «✅ Оплатил» на свежем сообщении с реквизитами.",
            reply_markup=REPLY_KB,
        )
        return
    ...
    if not o or int(o.get("user_id") or 0) != uid:
        await q.message.reply_text(
            "Заказ не найден. Нажмите «✅ Оплатил» на сообщении с реквизитами ещё раз.",
            reply_markup=REPLY_KB,
        )
        return
    _refresh_unpaid_order_payment(
...
def _resolve_payment_step_order_id(
    uid: int, ud: Optional[dict] = None, msg: Optional[Message] = None
) -> Optional[int]:
    """Заказ на шаге оплаты: in-memory сессия → последний неоплаченный → Redis."""
    ud_d = ud if isinstance(ud, dict) else {}
    if msg is not None:
        from_reply = _proof_order_id_from_reply_context(uid, ud_d, msg)
        if from_reply is not None:
    ...
    if latest is not None:
        oid_i = int(latest)
        o = ORDERS.get(oid_i)
        if isinstance(o, dict) and int(o.get("user_id") or 0) == int(uid):
            if (
                str(o.get("payment_pending_method") or "").strip()
                or ud_d.get("payment_pending_method")
                or _user_state_get(uid, "proof_deadline") is not None
            ):
                return oid_i
        return oid_i
    _refresh_orders_state_from_redis()
    return _resolve_proof_order_id_aggressive(uid, ud_d, msg)


async def on_customer_proof_media(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Единая точка входа для фото/картинок клиента — сначала ответ клиенту, потом Redis."""
    log = logging.getLogger(__name__)
    msg = update.effective_message
    if not msg or not msg.from_user:
        raise ApplicationHandlerStop
    ...
    if not file_id:
        if _user_wants_proof_upload(uid, ud, msg):
            ...
            await msg.reply_text(
                "Сначала выберите способ оплаты, затем пришлите скрин оплаты.",
                reply_markup=REPLY_KB,
            )
        raise ApplicationHandlerStop
    ...
    try:
        ...
        await msg.reply_text(
            "Сначала выберите «💳 Карта» / «📱 Перевод» на шаге оплаты, "
            "затем пришлите скрин. Кнопку «✅ Оплатил» можно не нажимать.",
            reply_markup=REPLY_KB,
        )
    except ApplicationHandlerStop:
        raise
...
async def _on_user_photo_body(
    ...
    if uid and file_id_peek and not shipping_photo:
        ...
        if _find_latest_unpaid_order_id(uid) is not None:
            ...
            return
        await msg.reply_text(
            "Сначала нажмите «✅ Оплатил» на сообщении с реквизитами, затем пришлите скрин.",
            reply_markup=REPLY_KB,
        )
        return
    if proof_oid is not None:
...
async def _on_payment_proof_photo_body(
    ...
    if not uid:
        return
    oid_raw = _resolve_payment_step_order_id(uid, ud, msg)
    if oid_raw is None:
    ...
    if not _proof_order_owned_by_user(uid, oid):
        ...
        if isinstance(ud, dict):
            ud.pop("awaiting_proof", None)
        await msg.reply_text(
            "Этот чек не привязан к вашему заказу. Нажмите «✅ Оплатил» и пришлите скрин снова.",
            reply_markup=REPLY_KB,
        )
        return
    ...
    if not o or int(o.get("user_id") or 0) != int(uid):
        ...
        if isinstance(ud, dict):
            ud.pop("awaiting_proof", None)
        await msg.reply_text(
            "Не удалось привязать чек к заказу. Нажмите «✅ Оплатил» на шаге оплаты и пришлите скрин снова.",
            reply_markup=REPLY_KB,
        )
        return
    _user_state_pop(uid, "postpaid_thread_oid")
    ...
    try:
        ...
        shipped = True
        log.info(
            "payment_proof_photo ok uid=%s oid=%s awaiting_shipping prompt_sent=1",
            uid,
            oid,
        )
    except Exception:
    ...
    if not shipped:
        log.error(
            "payment_proof_photo shipping prompt FAILED uid=%s oid=%s", uid, oid
        )
        try:
            await msg.reply_text(MSG_POSTPAID_SUBMIT_FAIL, reply_markup=REPLY_KB)
        except Exception:
...
async def on_postpaid_submit(
    ...
    if not _postpaid_merged_details(o):
        ...
        if q.message:
            try:
                await q.message.reply_text(MSG_POSTPAID_NEED_DETAILS)
            except Exception:
                pass
        return
    try:
    ...
    if not ok:
        if q.message:
            await _reply_postpaid_finalize_to_client(q.message, ok=ok, reason=reason)
        return
    if q.message:
        await _reply_postpaid_finalize_to_client(q.message, ok=True, reason="ok")
    raise ApplicationHandlerStop
...
async def _try_handle_postpaid_shipping_text(
    ...
    *,
    ...
    if thread_oid is None:
        asp = _user_state_get(uid, "awaiting_postpaid_shipping")
        if asp is not None:
            oid_i = int(asp)
            if _recover_payment_proof_on_order(uid, oid_i) or _shipping_session_valid(uid, oid_i):
                thread_oid = oid_i
                _bind_postpaid_thread_oid(uid, user_data, oid_i, persist=False)
                log.info("%s recovered awaiting_postpaid uid=%s oid=%s", log_prefix, uid, oid_i)
    if thread_oid is None:
        if (
            _user_state_get(uid, "postpaid_thread_oid") is not None
            or _user_state_get(uid, "awaiting_shipping_oid") is not None
            or _user_state_get(uid, "awaiting_postpaid_shipping") is not None
        ):
            try:
                await msg.reply_text(MSG_POSTPAID_RESEND_PROOF, reply_markup=REPLY_KB)
            except Exception:
...
async def on_postpaid_shipping_text(
    ...
    try:
        ...
        logging.getLogger(__name__).exception(
            "postpaid shipping handler failed uid=%s",
            msg.from_user.id if msg and msg.from_user else 0,
        )
        if msg:
            try:
                await msg.reply_text(MSG_POSTPAID_SUBMIT_FAIL, reply_markup=REPLY_KB)
