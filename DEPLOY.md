...
`UPSTASH_REDIS_REST_URL` и `UPSTASH_REDIS_REST_TOKEN` используются ботом для долговременного сохранения заказов клиента, админских заказов, корзин, избранного, бонусов и служебных состояний между деплоями. Для Render/серверов без persistent disk эти переменные обязательны: если Redis не задан, бот использует локальный `bot_state.json`, который может пропадать после деплоя.
