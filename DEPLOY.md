...
| `ILLUCARDS_CART_CLEAR_ON_PROOF_URL` | Опционально: URL на сайте (Vercel) для **POST** после того, как клиент прислал скрин оплаты в бот — очистить корзину в БД/сессии сайта (см. раздел «Корзина и чек» ниже) |
...
- **В боте** позиции корзины и подсказки цен с сайта (`site_cart_grand_*`) сбрасываются **после успешной отправки клиентом скрина оплаты** (не при создании заказа и не при нажатии админом «Принять заказ»). Подтверждение оплаты админом только списывает бонусы и шлёт чек — корзина к этому моменту уже пустая, если заказ оформлен с флагом «очистить после оплаты».
- **На сайте** бот сам корзину не чистит: нужен ваш обработчик. Если задан **`ILLUCARDS_CART_CLEAR_ON_PROOF_URL`**, после скрина оплаты бот выполняет **POST** на этот URL с заголовком **`Authorization: Bearer …`** (тот же секрет, что **`ILLUCARDS_ORDER_UPDATE_SECRET`**, если он задан в окружении бота) и JSON-телом:
